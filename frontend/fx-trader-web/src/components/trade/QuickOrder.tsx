...
export const QuickOrder = () => {
  ...
  const quickTrade = async (side: 'buy' | 'sell', volume: number) => {
    ...
    } catch { message.error('Order failed'); }
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <Button type="primary" danger block onClick={() => quickTrade('sell', 0.01)}>SELL {quote?.bid.toFixed(5)}</Button>
        <Button type="primary" block onClick={() => quickTrade('buy', 0.01)}>BUY {quote?.ask.toFixed(5)}</Button>
      </div>
      <Divider>Volumes</Divider>
      <Space wrap>{[0.01, 0.05, 0.1, 0.5, 1].map((v) => <Button key={v} size="small" onClick={() => quickTrade('buy', v)}>{v}</Button>)}</Space>
    </div>
  );
};
