  ...
  <el-card>
    <template #header
      ><div style="display: flex; justify-content: space-between; align-items: center">
        <span>Order List</span><ExportButton :api="orderApi.getList" /></div
    ></template>
    <SearchForm v-model="query" @search="handleSearch" @reset="handleReset">
      <el-form-item label="Symbol"
        ><el-input v-model="query.symbol" placeholder="Symbol" clearable
      /></el-form-item>
      <el-form-item label="Status"
        ><el-select v-model="query.status" placeholder="Status" clearable
          ><el-option label="Pending" value="pending" /><el-option
            label="Filled"
            value="filled" /><el-option label="Cancelled" value="cancelled" /></el-select
      ></el-form-item>
    </SearchForm>
    <DataTable
      v-model:page="page"
...
const { loading, data, total, page, limit, query, fetchData, handleSearch, handleReset } = useTable(
