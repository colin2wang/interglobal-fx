<template>
  <el-card title="Positions">
    <el-table :data="positions" v-loading="loading">
      ...
      <el-table-column prop="profit" label="P/L"><template #default="{row}"><span :style="{color:row.profit>=0?'#67c23a':'#f56c6c'}">{{row.profit?.toFixed(2)}}</span></template></el-table-column>
    </el-table>
  </el-card>
</template>
...
const positions = ref([]);
onMounted(async () => { loading.value = true; try { const res = await request.get('/admin/positions'); positions.value = res.data; } finally { loading.value = false; } });
</script>
